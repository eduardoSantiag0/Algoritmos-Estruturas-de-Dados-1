...
PONT buscarChave (PONT raiz, int ch) 
{
    ...
    if (ch > raiz->chave) 
        return (buscarChave(raiz->esq, ch)); 
    else 
...
PONT remover(PONT raiz, int ch)
{
    ...
    if (noParaRemover->esq == NULL || noParaRemover->dir == NULL) {
        if (noParaRemover->esq == NULL) 
            filhoPromovido = noParaRemover->dir;
        else 
    ...
    if (ch < pai->chave) 
        pai->esq = filhoPromovido;
    else 
        pai->dir = filhoPromovido;
...
int main (void) {
    ...
    PONT p = criarNo(23); 
    raiz = adicionar(raiz, p);
    PONT q = criarNo(13); 
    raiz = adicionar(raiz, q);
    PONT r = criarNo(44); 
    raiz = adicionar(raiz, r);
    PONT l = criarNo(54); 
    raiz = adicionar(raiz, l); 
    PONT m = criarNo(8); 
    raiz = adicionar(raiz, m); 
    PONT n = criarNo(18); 
    raiz = adicionar(raiz, n); 
    PONT y = criarNo(30); 
    raiz = adicionar(raiz, y); 
